...
const STAKING_OPTIONS: Record<string, any> = {
    "30D-1": { duration: 30, apr: 1, rewardValue: "$200", agentAmount: 1, image: "/agents/cosmo.png" },
    "30D-4": { duration: 30, apr: 4, rewardValue: "$800", agentAmount: 4, image: "/agents/trump.png" },
    "60D-10": { duration: 60, apr: 10, rewardValue: "$2000", agentAmount: 10, image: "/agents/monkey.png" },
};

export default function StakingDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const router = useRouter();
    const resolvedParams = use(params);
    const id = resolvedParams.id.toUpperCase();
    const option = STAKING_OPTIONS[id] || STAKING_OPTIONS["30D-1"];
    ...
    return (
        <div className="p-8 pb-32">
            <div className="flex items-center gap-4 mb-8">
                ...
                <h1 className="text-white text-2xl font-black">Stake $SUI - {option.duration} Days</h1>
            </div>

            <div className="flex gap-8">
                {/* Stake Form Section */}
                <div className="flex-1 bg-[#111] rounded-3xl border border-white/5 p-8 space-y-8">
                    <div className="space-y-4">
                        <h3 className="text-white font-bold text-lg uppercase tracking-wider">Lock duration</h3>
                        <div className="flex gap-4">
                            {Object.entries(STAKING_OPTIONS).map(([key, opt]) => (
                                <button
                                    key={key}
                                    onClick={() => router.push(`/staking/${key.toLowerCase()}`)}
                                    className={cn(
                                        ...
                                        id === key ? "bg-[#ff0066] border-[#ff0066] text-white" : "bg-white/5 border-white/5 text-gray-400 hover:bg-white/10"
                                    )}
                                >
                                    ...
                                    <div className={cn(
                                        ...
                                        id === key ? "bg-white/20" : "bg-black/50"
                                    )}>
                                        {opt.agentAmount}
                                    </div>
                                </button>
